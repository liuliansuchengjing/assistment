...
def test_epoch(model, validation_data, graph, hypergraph_list, kt_loss, k_list=[5, 10, 20]):
    """Epoch operation in evaluation phase (Path-level, Scheme A).

    Scheme A: For a given k, only evaluate samples/windows where the future ground-truth length >= k.
    Here we compute path-level metrics by comparing a greedy top-1 predicted path of length k
    with the next k ground-truth interactions.

    Returns:
        scores: dict of averaged metrics
        auc_test, acc_test, kt_prec_test, kt_rec_test, kt_f1_test: KT metrics lists
    """
    model.eval()
    auc_test = []
    acc_test = []
    ...
    with torch.no_grad():
        for i, batch in enumerate(validation_data):
            ...
            # forward
            pred, pred_res, kt_mask = model(tgt, tgt_timestamp, tgt_idx, ans, graph, hypergraph_list)
            ...
            # pred is flattened: (B*T, V) -> reshape to (B, T, V)
            B = tgt.size(0)
            T = gold_seq.shape[1]
            pred_seq = pred.view(B, T, pred.size(-1)).detach().cpu().numpy()

            # Path-level evaluation (Scheme A)
            scores_batch, win_len = metric.compute_path_metric(pred_seq, gold_seq, k_list)
            n_total_windows += win_len

            if win_len == 0:
                continue

            for k in k_list:
                scores['acc@' + str(k)] += scores_batch['acc@' + str(k)] * win_len
                scores['exact@' + str(k)] += scores_batch['exact@' + str(k)] * win_len
                scores['precision@' + str(k)] += scores_batch['precision@' + str(k)] * win_len
                scores['recall@' + str(k)] += scores_batch['recall@' + str(k)] * win_len
                scores['f1@' + str(k)] += scores_batch['f1@' + str(k)] * win_len
                scores['map@' + str(k)] += scores_batch['map@' + str(k)] * win_len
                scores['NDCG@' + str(k)] += scores_batch['NDCG@' + str(k)] * win_len

    if n_total_windows == 0:
        return scores, auc_test, acc_test, kt_prec_test, kt_rec_test, kt_f1_test

    for k in k_list:
        scores['acc@' + str(k)] /= n_total_windows
        scores['exact@' + str(k)] /= n_total_windows
        scores['precision@' + str(k)] /= n_total_windows
        scores['recall@' + str(k)] /= n_total_windows
        scores['f1@' + str(k)] /= n_total_windows
        scores['map@' + str(k)] /= n_total_windows
        scores['NDCG@' + str(k)] /= n_total_windows
