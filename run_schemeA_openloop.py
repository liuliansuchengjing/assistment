...
def test_model(MSHGAT, data_path, mode="teacher_forcing", max_starts_per_seq=None):
    ...
    model.cuda()
    kt_loss = kt_loss.cuda()
    scores, auc_test, acc_test, kt_p, kt_r, kt_f1 = test_epoch(model, test_data, relation_graph, hypergraph_list, kt_loss, k_list=[1,3, 5, 7, 9], mode=mode, max_starts_per_seq=max_starts_per_seq)
    print('  - (Test) ')
    for metric in scores.keys():
        print(metric + ' ' + str(scores[metric]))
    print('auc_test: {:.10f}'.format(np.mean(auc_test)),
...
if __name__ == "__main__":
    ...
    test_model(model, opt.data_name, mode="open_loop", max_starts_per_seq=20)